...
**Описание:**
Создать чат техподдержки (если не существует). Если чат уже существует, возвращает существующий.
...
**Описание:**
Получить историю сообщений чата. Используется при открытии чата для загрузки истории.
...
**Описание:**
Отправить сообщение через HTTP API (альтернатива WebSocket).
...
**Примечание:** Сообщение также отправляется через Socket.io всем участникам чата.
...
**Примечание:** Событие также отправляется через Socket.io всем участникам чата.
...
**Описание:**
Получить все чаты в системе (только для админов).
...
**Ответ (200):**
Аналогично GET `/chats`, но со всеми чатами в системе.
...
JWT токен передается в параметре `auth.token` или в заголовке `Authorization: Bearer <token>`.
...
socket.on('connect_error', (error) => {
  // Ошибка подключения (включая ошибки аутентификации)
...
- Отправляет событие `joined_chat` с подтверждением

**Ответ сервера (`joined_chat`):**
```json
{
  "success": true,
  "chatId": "uuid",
...
**Примечание:** `messageType` по умолчанию `"text"`

**Сервер:**
- Проверяет права доступа (пользователь должен быть участником чата)
- Сохраняет сообщение в БД
- Отправляет событие `new_message` всем участникам комнаты `chat:{chatId}`
- TODO: Отправляет push-уведомления (кроме отправителя и админов для support)

**Ответ сервера (`new_message`):**
```json
{
  "success": true,
  "id": "uuid",
...
```json
{
  ...
  "error": "Database query timeout after 25000ms",
  "errorDetails": {
    "name": "Error",
    "code": "ETIMEDOUT",
    "sqlMessage": null
...
**Сервер:**
- Сохраняет запись в `message_reads` (или обновляет существующую)
- Отправляет событие `message_read` всем участникам комнаты `chat:{chatId}`

**Ответ сервера (`message_read`):**
```json
{
  "success": true,
  "messageId": "uuid",
...
```json
{
  ...
  "error": "messageId и chatId обязательны",
  "errorDetails": {
    "name": "Error",
    "code": null,
    "sqlMessage": null
...
##### `joined_chat` - Подтверждение присоединения к чату

См. выше в разделе `join_chat`.

##### `new_message` - Новое сообщение

См. выше в разделе `send_message`.

##### `message_read` - Сообщение прочитано

См. выше в разделе `mark_as_read`.

##### `user_typing` - Пользователь печатает

См. выше в разделе `typing`.

##### `error` - Ошибка

Возвращается при любой ошибке:
...
```json
{
  ...
  "errorDetails": {
    ...
    "sqlMessage": "Table 'database.chats' doesn't exist"
  }
}
```

**Важно:** Все ошибки возвращаются в JSON формате через событие `error`. Нет логирования в файлы - все ошибки видны клиенту.
...
9. **WebSocket:** Используйте Socket.io для чатов, polling - основной транспорт для Passenger на Beget
