...
function buildSystemInstruction(answerLanguage) {
  ...
  return [
    ...
    answerLanguage === 'ru'
      ? 'Любой ответ про донаты, кто получит деньги, донейшен или выплату по донату: обязательно одним коротким предложением уточни, что деньги на карту получит только тот, у кого в профиле полностью подключён и настроен Stripe Connect. Не расписывай отдельным абзацем, что автор точки «не получает только за создание», если пользователь прямо не спорит про роль автора.'
      : 'For any donation, who-gets-paid, or donation-payout question: always add one short sentence that cash goes only to someone with a fully connected Stripe Connect profile. Do not add a long aside that the pin creator is not paid just for creating unless the user argues creator vs executor.',
    answerLanguage === 'ru'
