...
- **`job_id`** (строка UUID) — сохраните в **`sessionStorage` / `localStorage`** для опроса после перезагрузки вкладки;
- **`total`** — сколько id принято в задачу;
- **`chunk_size`** — размер одного внутреннего чанка (= **`EARTHDAY_BULK_CHUNK_MAX`**, по умолчанию 5).

**Ошибки:** **`400`** — валидация тела; **`500`** — БД и т.п.

#### `GET /earthday-cleanups-admin/bulk-create-jobs/:jobId`

Статус **своей** задачи (только если **`created_by`** совпадает с пользователем JWT).
...
| `objectids_preview` | Укороченный превью списка id (для отладки в UI) |
...
Последние задачи **текущего** суперадмина (если пользователь закрыл вкладку и потерял **`job_id`**).
...
1. После **`POST .../async`** сохранить **`job_id`** в **`localStorage`** (например ключ `earthday_bulk_job_id`).
2. Опрос **`GET .../bulk-create-jobs/:jobId`** каждые **3–10 с**; при **`document.visibilityState === 'hidden'`** — реже или с паузой, при возврате на вкладку — сразу один запрос.
3. При **`status === 'done'`** или **`failed`** — остановить таймер; при **`done`** можно очистить сохранённый **`job_id`**.
4. При открытии экрана Earth Day: если в storage есть **`job_id`** — запросить статус; если нет — **`GET .../bulk-create-jobs`** и показать последнюю незавершённую задачу при необходимости.
