...
Полученный `data.url` подставляйте в поле `image_url` при создании/редактировании новости или в другие сущности.
...
**Базовый путь:** `/api/news` — как у станций переработки (`/api/recycling-stations`): один путь для приложения и админки; админские операции (POST, PUT, DELETE) требуют авторизацию и права администратора.
...
| `short_description` | string \| null | Краткое описание / подзаголовок статьи (до 500 символов) |
...
Те же пути, что и для приложения. Заголовок **`Authorization: Bearer <admin_token>`** обязателен для POST, PUT, DELETE.

#### Список новостей (админ)

**GET** `/news?page=1&limit=20`

Тот же эндпоинт, что и для приложения. Возвращает все новости с полями `likes_count`, `updated_at` и т.д.

---

#### Одна новость (админ, без увеличения просмотров)

**GET** `/news/:id?skip_view=1`

Если передан query-параметр `skip_view=1` и пользователь — администратор, счётчик просмотров не увеличивается (удобно при открытии новости для редактирования).
...
- `title` — обязательно.
- `short_description` — необязательно; краткое описание/подзаголовок, до 500 символов.
- `text` — обязательно.
...
```json
{
  ...
  "short_description": "Краткое описание / подзаголовок",
