...
5. **Автоматическое удаление**
   - Если в течение **7 дней** (недели) к заявке никто не присоединился:
     ...
     - Если заявка не была продлена, через **8 дней** (7 дней + 1 день ожидания):
...
4. **`expires_at`** (datetime, опционально)
   ...
   - Автоматически устанавливается при создании: `created_at + 7 дней`
   - При продлении заявки: `expires_at += 7 дней`
...
**Описание:**
Продление заявки типа `wasteLocation` еще на неделю. Доступно только для заявок со статусом `new`, которые еще не истекли и не были продлены ранее.
...
- `expires_at` увеличивается на 7 дней
