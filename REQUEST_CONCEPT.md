...
5. **Автоматическое удаление**
   - Если в течение **7 дней** (недели) к заявке никто не присоединился:
     ...
     - Пользователю уходит соответствующий пуш-уведомление
...
2. **Удаление неактивных заявок (каждые 24 часа)**
   - Найти все заявки со статусом `new`
   - Для каждой заявки проверить: если `created_at + 7 дней < текущее время`
   - Если условие выполнено:
