...
- **Vision (OpenRouter)** при bulk-create и при **переиспользовании кэша** (`EARTHDAY_VISION_FILTER`, по умолчанию вкл. при `OPENROUTER_API_KEY`): отсеивает музеи, экспонаты, banding и пр.; **reject** → файл удаляется из `/uploads`, строки — из `earthday_image_cache` и `request_creation_gallery` (файл на диске **сохраняется**, если URL уже в `photos_before`/`photos_after` заявки). Скрипт разовой чистки кэша: `node scripts/earthday_purge_bad_cache_images.js` (`--dry-run`, `--limit=N`).
