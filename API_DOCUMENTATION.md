...
API для мультиязычной ленты новостей. Контент при создании/редактировании задаётся одним полем `content` и языком оригинала `source_lang`; сервер переводит на все поддерживаемые локали и сохраняет в БД. При выдаче обязателен параметр `locale` — в ответе возвращаются уже переведённые `title`, `short_description`, `text` для выбранного языка.
...
|----------|--------------|----------|
| `locale` | **Да** | Один из: `en`, `ru`, `es`, `ar`, `zh`, `hi`, `fr`, `pt`, `he`, `de`. Без него — 400. |
...
**Ошибка (400):** отсутствует или недопустимый `locale`.
...
**Query:** `locale` — **обязателен**, один из поддерживаемых кодов.
...
| `image_url` | Нет | URL картинки (http/https); пустая строка или отсутствие — `null`. |
