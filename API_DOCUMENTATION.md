...
По нажатию в админке: удаляет строки с **`cleanup_date` раньше чем `now + 24 часа`**; подтягивает из ArcGIS до **1000** заявок в окне дат **`now+24ч` … `now+7 суток`**; вставляет/обновляет по **`objectid`**. Записи **без `start_time`**, без точки в `geometry`, с координатами вне WGS84 или с заглушкой **(0, 0)** **пропускаются** — в таблицу не попадают. Счётчики: **`skippedNoStartTime`**, **`skippedNoCoordinates`**. Для сохранённых строк по валидным **`lat` / `lng`** пересчитываются и пишутся в БД **`continent`**, **`country`**, **`location_hint`** (см. описание **GET** выше).
...
**Чанки по 5:** клиент шлёт **не больше 5** **`objectid`** за один запрос, дожидается ответа и затем следующий чанк — так удобно показывать **прогресс**. На сервере **нет группировки** по адресу: для **каждой** строки свой запрос к Wikimedia Commons, свои скачивания, сжатие **≤ 1 МБ (JPEG)**, запись в **`request_creation_gallery`** и **`photos_before`**; после успеха — **`used_for_internal_request = 1`**.
...
**Ответ `200`:** если создана **хотя бы одна** заявка — **`success: true`**, **`requested_count`**, **`created_count`**, **`created`**, **`errors_count`**, **`errors`**, **`semantics: "chunk_sequential"`**. Часть id в том же чанке может попасть в **`errors`** — это нормально.

**Ответ `422`:** в теле запроса были **`objectid`**, но **`created_count === 0`** (ни одна заявка не создана). **`success: false`**, текст в **`message`**, детали в **`errorDetails`**: **`requested_count`**, **`created_count`**, **`errors_count`**, массив **`errors`**. Клиент может показать ошибку чанка и не продолжать автоматически (или повторить чанк после исправления).
...
Коды в **`errors`**, среди прочих: **`NOT_FOUND`**, **`ALREADY_USED`**, **`BAD_COORDS`**, **`BAD_DATE`**, **`BAD_START_TIME`**, **`NO_SEARCH`**, **`WIKIMEDIA`**, **`NO_COMMONS_IMAGES`**, **`IMAGE_PIPELINE`**, **`CREATE_REQUEST`**.
...
- Уклон в **природу** и минусы по городу/спутнику задаются **короткой** строкой (лимит Commons: **не более ~300 символов** «поискового текста» без `nearcoord:` и `filetype:bitmap`; длинный адрес/`location_hint` при необходимости **обрезается**). Расширенный отсев по-прежнему делается по **имени файла** (здания, орбита, инфра и т.д.). Явные **iNaturalist** / виды в названии по возможности сохраняются.
